        ...
        uploadXhr(xhr, _file, body) {
            ...
            return new Promise((resolve, reject) => {
                ...
                let fn = e => {
                    ...
                    if (xhr.response) {
                        data.resultBlob = xhr.response;
                        var responseType = xhr.response.type;

                        //将Blob 对象转换成字符串
                        var reader = new FileReader();
                        reader.readAsText(data.resultBlob, "utf-8");
                        reader.onload = e => {
                            var readerResult = reader.result;
                            let contentType = xhr.getResponseHeader(
                                "Content-Type"
                            );
                            if (
                                contentType &&
                                ...
                                data.response = JSON.parse(readerResult);
                            } else {
                                data.response = readerResult;
                            }
