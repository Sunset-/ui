...
export default {
    ...
    computed: {
        ...
        setting() {
            ...
            if (check == "single" || check == "multiple") {
    ...
    methods: {
        init() {
            ...
            this.$el.id = "xui-stree-" + ++treeCounter;
            Promise.resolve()
                .then(() => {
                    if (this.cacheInitNodes) {
                        return this.cacheInitNodes;
                    } else if (this.treeNodes) {
                ...
                .then(nodes => {
                    ...
                    if (nodes) {
                        nodes.forEach(n => {
                            this.cacheInitNodesMap[n[this.idPlace]] = n;
                        });
                    }
        ...
        cancelSelectedNode(data) {
            this.ztreeObj.cancelSelectedNode(data ? this.getNodeByData(data) : void 0);
        },
        //勾选节点
        checkNode(node, toCheck) {
            var treeNode = this.getNodeByData(node);
            this.ztreeObj.checkNode(treeNode, toCheck);
        },
        //刷新树全部节点
        refresh(nodes) {
            this.cacheInitNodes = null;
            this.cacheInitNodesMap = {};
        ...
        //显示节点
        showNodes(nodesMap) {
            this.ztreeObj.showNodes(this.ztreeObj.getNodesByParam("isHidden", true));
            if (nodesMap === true) {
                return;
            }
            this.ztreeObj.hideNodes(
                this.cacheInitNodes.filter(item => !nodesMap[item[this.idPlace]]).map(item => this.getNodeByData(item))
            );
        },
        getParentNodes(data) {
            var tree = this.ztreeObj;
            var node = this.getNodeByData(data);
            var child = node;
            var parents = [];
            while (true) {
                var parent = child.getParentNode();
                if (parent) {
                    parents.unshift(parent.data || parent);
                    child = parent;
                } else {
                    break;
                }
