<template>
	<div :class="['xui-treeselect xui-treeselect-style',opened?'focus':'',safeOptions.className]">
		...
		<div ref="dropdown" v-if="inited" v-show="opened" class="xui-treeselect-tree" :style="dropdownStyle">
			<treeselect-stree ref="tree" :options="treeOptions" :nodes="treeNodes" @checked="checkedHandle" @selected="selectedHandle"></treeselect-stree>
		</div>
	</div>
</template>
<script>
...
import TreeselectStree from "../../base/stree";
	...
	computed: {
		...
		treeOptions() {
			var enumType = this.safeOptions.enum,
				treeOptions = this.safeOptions.treeOptions;
			if (enumType) {
				return {
					autoExpand: this.autoExpand,
					check: this.isMultiple ? "multiple" : false,
					setting: {
						data: {
							key: {
								name: "text",
